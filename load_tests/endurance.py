...
class EnduranceUser(HttpUser):
    ...
    @task(1)
    def update_profile_endurance(self):
        """Sustained profile updates"""
        profile_data = {
            "full_name": "Endurance User",
            "phone": "+1234567890",
            "location": "Endurance City",
            "headline": "Endurance Engineer",
            "skills": ["python", "locust", "performance"],
        }
        with self.client.put("/api/profile", json=profile_data, catch_response=True) as response:
            if response.status_code == 200:
                try:
                    data = response.json()
                    if not isinstance(data, dict) or "id" not in data:
                        response.failure("Invalid profile update response")
                except json.JSONDecodeError:
                    response.failure("Invalid JSON response")
            elif response.status_code != 404: # It's ok if the profile is not found
                response.failure(f"Status code {response.status_code}")
