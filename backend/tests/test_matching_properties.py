...
class TestMatchingProperties:
    ...
    async def test_score_with_different_skill_sets(
        self, job_description, candidate_skills
    ):
    ...
    async def test_skill_match_correlation(self, job_description, skills):
        ...
        job_with_skills = create_mock_job(
            description=f"{job_description} {' '.join(skills)}"
        )
    ...
    @pytest.mark.asyncio
    @given(st.text(min_size=10, max_size=300), st.booleans())
    async def test_openai_fallback_behavior(self, job_description, api_available):
        ...
        with patch(
            "backend.services.matching.embedding_service.is_available"
        ) as mock_available:
    ...
    @pytest.mark.asyncio
    @given(st.lists(st.text(min_size=2, max_size=20), min_size=3, max_size=3))
    async def test_bm25_skill_correlation(self, skills):
        """Test that BM25 score correlates with skill match"""
        profile = create_mock_profile(skills=skills)
        job_with_skills = create_mock_job(
            description=f"We need {', '.join(skills)} developers"
        )
        job_without_skills = create_mock_job(
            description="We need developers with no specific skills"
        )
    ...
    async def test_bm25_experience_correlation(
        self, job_description, skills, years_experience
    ):
