...
class DomainRateLimiter:
        ...
        try:
            ...
            for domain in domains:
                self.domain_limits[domain.host] = {
                    'rate_limit_policy': json.loads(domain.rate_limit_policy) if domain.rate_limit_policy else self._default_rate_limit(),
    ...
    def is_rate_limited(self, domain: str) -> bool:
        ...
        requests_last_minute = sum(1 for ts in self.request_timestamps[domain] if ts >= minute_ago)
        requests_last_hour = sum(1 for ts in self.request_timestamps[domain] if ts >= hour_ago)
        requests_last_day = sum(1 for ts in self.request_timestamps[domain] if ts >= day_ago)
        
        if (requests_last_minute >= rate_limit['requests_per_minute'] or
            requests_last_hour >= rate_limit['requests_per_hour'] or
    ...
    def get_wait_time(self, domain: str) -> float:
        ...
        earliest_request = min(ts for ts in self.request_timestamps[domain] if ts >= minute_ago)
