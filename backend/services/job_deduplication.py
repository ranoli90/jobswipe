...
def calculate_job_similarity(job1: Job, job2: Job) -> Dict[str, float]:
    ...
    company_score = (
        fuzz.token_sort_ratio(company1, company2) if company1 and company2 else 0
    )
...
def is_duplicate(job1: Job, job2: Job) -> Tuple[bool, Dict[str, float]]:
    ...
    is_dup = scores["title"] >= MIN_TITLE_SIMILARITY and (
        scores["company"] >= MIN_COMPANY_SIMILARITY
        or scores["description"] >= MIN_DESCRIPTION_SIMILARITY
    )
...
def deduplicate_jobs(
    jobs: List[Job],
) -> Tuple[List[Job], List[List[Tuple[Job, float]]]]:
    """
