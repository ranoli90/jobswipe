...
class JobIngestionService:
                ...
                async with session.get(config["url"], timeout=aiohttp.ClientTimeout(total=10)) as response:
                    ...
                    for entry in feed.entries:
                        try:
                            ...
                            if any(job_type.lower() in entry.title.lower() for job_type in self.JOB_TYPES):
                                # Extract job details
                                job = {
                                    "id": entry.id if hasattr(entry, "id") else hash(entry.link),
                                    "title": entry.title,
                                    "company": self.extract_company_from_rss(entry),
                                    "location": self.extract_location_from_rss(entry),
                                    "description": self.extract_description_from_rss(entry),
                                    "url": entry.link,
                                    "source": config.get("category", "rss"),
                                    "salary_range": self.extract_salary_from_rss(entry),
                                    "type": config.get("category", "software"),
                                    "created_at": entry.published if hasattr(entry, "published") else datetime.now().isoformat()
                                }
    ...
    async def ingest_scraped_jobs(self, config: Dict) -> List[Dict]:
        ...
        for company_config in config["companies"]:
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.get(company_config["url"], timeout=aiohttp.ClientTimeout(total=10)) as response:
                        response.raise_for_status()
                        content = await response.text()
                        ...
                        for link in job_links:
                            try:
                                ...
                                if not job_url.startswith("http"):
                                    job_url = f"{company_config['url'].rstrip('/')}{job_url}"
                                    
                                # Extract job title from link text or attribute
                                job_title = link.get_text(strip=True)
                                
                                # Check if job title contains any of the desired job types
                                if any(job_type.lower() in job_title.lower() for job_type in self.JOB_TYPES):
                                    job = {
                                        "id": hash(job_url),
                                        "title": job_title,
    ...
    async def process_jobs_batch(self, jobs: List[Dict]):
        ...
        logger.info(f"Batch processed: {success_count} succeeded, {failed_count} failed")
    ...
    async def run_periodic_ingestion(self, interval_seconds: int = 3600):
        ...
        logger.info(f"Starting periodic job ingestion with interval: {interval_seconds} seconds")
    ...
    async def run_once(self):
        ...
        try:
            ...
            if jobs:
                ...
                logger.info(f"Processing complete: {success} succeeded, {failed} failed")
            else:
                logger.info("No new jobs to ingest")
