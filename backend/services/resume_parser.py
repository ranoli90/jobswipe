...
def parse_work_experience(text: str) -> list:
    ...
    positions = []
    job_titles = ["software engineer", "developer", "engineer", "analyst", "manager", "director"]
...
def parse_skills(text: str) -> list:
    ...
    common_skills = [
        ...
        "sql", "postgres", "mysql", "mongodb", "redis", "aws", "azure", "gcp"
    ]
...
def parse_resume(file_content: bytes, filename: str) -> dict:
    ...
    try:
        ...
        logger.info(f"Successfully parsed resume: {ai_parsed_data.get('full_name', 'Unknown')}")
