...
def main():
    ...
    try:
        if args.validate:
            ...
            if issues:
                logging.error(f"Validation issues found in {latest_migration.name}:")
                for issue in issues:
                    logging.error(f"  - {issue}")
                sys.exit(1)
            else:
                logging.info(f"✅ Migration {latest_migration.name} validated successfully")

        else:
            # Create new migration
            migration_file = creator.create_migration(args.message, args.data_migration)

            logging.info(f"✅ Created migration: {migration_file}")
