...
async def fetch_rss_feed(feed_url: str) -> List[RSSJob]:
    ...
    try:
        ...
        logger.error(f"HTTP error fetching RSS feed {feed_url}: {e.response.status_code} - {e.response.text}")
        raise
...
def update_or_create_job(rss_job: RSSJob, db) -> Job:
    ...
    if existing_job:
        ...
        db.add(existing_job)
        logger.info(f"Updated RSS job: {normalized_job['title']} ({normalized_job['external_id']})")
        return existing_job
    else:
        # Create new job
        new_job = Job(**normalized_job)
        db.add(new_job)
        logger.info(f"Created new RSS job: {normalized_job['title']} ({normalized_job['external_id']})")
        return new_job
...
async def sync_rss_feed(feed_url: str) -> List[Job]:
    ...
    try:
        ...
        db.commit()
        logger.info(f"Successfully synced {len(synced_jobs)} jobs from RSS feed {feed_url}")
        return synced_jobs
