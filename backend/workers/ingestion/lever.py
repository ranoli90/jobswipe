...
async def fetch_lever_postings(org_slug: str) -> List[LeverJob]:
    ...
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            return [
                LeverJob(
                    ...
                    createdAt=datetime.fromtimestamp(job["createdAt"] / 1000) if job.get("createdAt") else datetime.utcnow(),
        ...
        logger.error(f"HTTP error fetching Lever postings for {org_slug}: {e.response.status_code} - {e.response.text}")
        raise
    except Exception as e:
        logger.error(f"Error fetching Lever postings for {org_slug}: {str(e)}")
        raise
...
def update_or_create_job(lever_job: LeverJob, db) -> Job:
    ...
    if existing_job:
        ...
        db.add(existing_job)
        logger.info(f"Updated Lever job: {normalized_job['title']} ({normalized_job['external_id']})")
        return existing_job
    else:
        # Create new job
        new_job = Job(**normalized_job)
        db.add(new_job)
        logger.info(f"Created new Lever job: {normalized_job['title']} ({normalized_job['external_id']})")
        return new_job
    ...
    try:
        ...
        if incremental:
            ...
            latest_updated = db.query(Job.updated_at).filter(
                Job.source == "lever",
                Job.raw_json.contains({"org_slug": org_slug})
        ...
        db.commit()
        logger.info(f"Successfully synced {len(synced_jobs)} jobs from Lever organization {org_slug}")
        return synced_jobs
        
    except Exception as e:
        db.rollback()
        logger.error(f"Error syncing Lever organization {org_slug}: {str(e)}")
        raise
    finally:
        db.close()
