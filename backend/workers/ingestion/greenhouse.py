...
async def fetch_greenhouse_board(board_token: str) -> List[GreenhouseJob]:
    ...
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            return [
                GreenhouseJob(
                    ...
                    updated_at=datetime.fromisoformat(job["updated_at"].replace("Z", "+00:00")) if job.get("updated_at") else datetime.utcnow(),
        ...
        logger.error(f"HTTP error fetching Greenhouse board {board_token}: {e.response.status_code} - {e.response.text}")
        raise
...
def normalize_greenhouse_job(greenhouse_job: GreenhouseJob) -> dict:
    ...
    return {
        ...
        "raw_json": json.loads(greenhouse_job.content) if greenhouse_job.content else {},
...
def update_or_create_job(greenhouse_job: GreenhouseJob, db) -> Job:
    ...
    if existing_job:
        ...
        db.add(existing_job)
        logger.info(f"Updated Greenhouse job: {normalized_job['title']} ({normalized_job['external_id']})")
        return existing_job
    else:
        # Create new job
        new_job = Job(**normalized_job)
        db.add(new_job)
        logger.info(f"Created new Greenhouse job: {normalized_job['title']} ({normalized_job['external_id']})")
        return new_job


async def sync_greenhouse_board(board_token: str, incremental: bool = True) -> List[Job]:
    """
    ...
    try:
        ...
        if incremental:
            ...
            latest_updated = db.query(Job.updated_at).filter(
                Job.source == "greenhouse",
        ...
        db.commit()
        logger.info(f"Successfully synced {len(synced_jobs)} jobs from Greenhouse board {board_token}")
        return synced_jobs
