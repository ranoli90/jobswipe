...
class GreenhouseAgent:
        ...
        try:
            async with async_playwright() as p:
                ...
                try:
                    ...
                    # Check for CAPTCHA
                    captcha_detected = await GreenhouseAgent._check_captcha(page, logger)
                    if captcha_detected:
                        logger.step("captcha_detected", {"url": apply_url})
                        return False, "CAPTCHA detected - requires human intervention"
                    
                    # Fill out personal information
                    await page.fill('input[name="first_name"]', profile.get("full_name", "").split()[0] if profile.get("full_name") else "")
                    await page.fill('input[name="last_name"]', " ".join(profile.get("full_name", "").split()[1:]) if profile.get("full_name") and len(profile.get("full_name").split()) > 1 else "")
                    await page.fill('input[type="email"]', profile.get("email", ""))
                    await page.fill('input[name="phone"]', profile.get("phone", ""))
                    logger.step("fill_personal_info", {"data": {
                    ...
                    await page.select_option('select[name="job_title"]', label="Software Engineer")
    ...
    async def _check_captcha(page, logger):
        ...
        try:
            ...
            captcha_indicators = [
                "captcha", "g-recaptcha", "reCAPTCHA", "security-check",
        ...
        try:
            async with async_playwright() as p:
                ...
                try:
                    ...
                    await page.select_option('select[name="experience"]', label="3-5 years")
                    
                    # Submit application
                    await page.click('button[type="submit"]')
                    await page.wait_for_selector("text=Application submitted", timeout=60000)
                    logger.step("submit", {"result": "success"})
