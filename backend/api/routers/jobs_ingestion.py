...
from backend.services.job_ingestion_service import job_ingestion_service, ingest_jobs_once
...
INGESTION_API_KEY = os.getenv("INGESTION_API_KEY", "default-api-key")
...
async def sync_greenhouse_board(
    ...
    try:
        from backend.workers.ingestion.greenhouse import sync_greenhouse_board as greenhouse_sync
...
async def sync_lever_postings(
    ...
    try:
        from backend.workers.ingestion.lever import sync_lever_postings as lever_sync
...
async def start_periodic_ingestion(
    ...
    logger.info(f"Starting periodic ingestion with interval: {request.interval_seconds} seconds")
...
async def get_ingestion_sources():
    ...
    try:
        return {
            "sources": [
                {"name": "Greenhouse", "type": "greenhouse", "url": "https://boards-api.greenhouse.io"},
