...
from backend.db.models import User, Job, UserJobInteraction, CandidateProfile
from backend.api.routers.auth import get_current_user
from backend.services.matching import get_personalized_jobs, get_job_matches_for_profile
import logging
import redis
import json
import os
    ...
    try:
        ...
        job_matches = [
            JobMatch(
                ...
                snippet=match["job"].description[:200] + "..." if len(match["job"].description) > 200 else match["job"].description,
                score=match["score"],
                metadata=MatchMetadata(
        ...
        # Cache the result
        redis_client.setex(cache_key, 300, json.dumps([match.dict() for match in job_matches]))

        return job_matches
        
    except Exception as e:
        logger.error(f"Error retrieving job matches for user {current_user.id}: {str(e)}")
        raise HTTPException(
...
async def get_feed(
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Failed to read from cache: {str(cache_error)}. Continuing to fetch fresh data.")
        ...
        job_cards = [
            JobCard(
                ...
                snippet=job_data["job"].description[:200] + "..." if len(job_data["job"].description) > 200 else job_data["job"].description,
        ...
        try:
            redis_client.setex(cache_key, 300, json.dumps([card.dict() for card in job_cards]))
        except Exception as cache_error:
            logger.warning(f"Failed to cache job feed: {str(cache_error)}. Continuing without caching.")
        
        return job_cards
        
    except Exception as e:
        logger.error(f"Error retrieving job feed for user {current_user.id}: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def swipe_job(
    ...
    existing_interaction = db.query(UserJobInteraction).filter(
