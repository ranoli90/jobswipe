...
from backend.db.models import User, ApplicationTask, Job, CandidateProfile
from backend.services.application_automation import application_automation_service
...
AUTOMATION_API_KEY = os.getenv("AUTOMATION_API_KEY", "default-automation-key")
...
class NotificationRequest(BaseModel):
    ...
    task_id: str
    notification_type: str  # "captcha_detected", "application_failed", "rate_limited", etc.
    message: str
...
async def auto_apply_to_job(
    ...
    if credentials.credentials != AUTOMATION_API_KEY:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid automation API key")
    ...
    try:
        ...
        db = next(get_db())
        task = db.query(ApplicationTask).filter(ApplicationTask.id == request.task_id).first()
...
async def generate_cover_letter(
    ...
    """
    try:
        logger.info(f"Generating cover letter for job {request.job_id} by user {current_user.id}")
...
async def regenerate_cover_letter(
    ...
    """
    try:
        logger.info(f"Regenerating cover letter for job {request.job_id} by user {current_user.id}")
...
async def send_notification(
    ...
    """
    try:
        logger.info(f"Sending {request.notification_type} notification for task {request.task_id} to user {current_user.id}")
